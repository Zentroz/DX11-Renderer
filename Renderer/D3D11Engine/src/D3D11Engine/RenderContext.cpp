...
void D3D11RenderContext::UpdateBuffer(Handle h, UINT byteWidth, void* data) {
...
void D3D11RenderContext::DrawGeometryIndexed(Handle handle) {
...
void D3D11RenderContext::BindBufferVS(uint32_t slot, Handle handle) {
	if (handle == InvalidHandle) return;
...
void D3D11RenderContext::BindBufferPS(uint32_t slot, Handle handle) {
	if (handle == InvalidHandle) return;
...
void D3D11RenderContext::BindTextureVS(uint32_t slot, Handle handle) {
...
void D3D11RenderContext::BindTexturePS(uint32_t slot, Handle handle) {
	D3D11Texture* tex =  resourceProvider->GetResource<D3D11Texture>(handle);
	if (!tex) {
...
void D3D11RenderContext::BindPipeline(const zRender::PipelineStateContainer& pipelineState) {
	...
	ID3D11RasterizerState* rasterizer = resourceProvider->GetRasterizerState();
